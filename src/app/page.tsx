...
const STEPS = [
  ...
  {
    num: "II",
    title: "35가지 질문을 함께 나눠요",
    desc: "한 분씩 부모님께 들려드리고, 부모님 말씀을 받아 적습니다. 챕터별로 나눠 진행하셔도 좋아요.",
  },
  {
    num: "III",
    title: "한 권으로 정성껏 엮어요",
          ...
          <div className="max-w-3xl mx-auto">
            <Reveal className="text-center mb-16">
              ...
              >
                한 권의 책,
                <br />
                <span className="tabular-nums">₩29,900</span>
              </h2>
              <p className="text-lg text-ink-soft leading-[1.7] max-w-md mx-auto">
                미리보기는 무료. 결제 후 7일 이내 환불 가능합니다.
              </p>
            </Reveal>
            ...
            <Reveal>
                ...
                {PRICING_COMPARISON.map((p) => (
                  ...
                  >
                    <div className="flex items-center justify-between gap-4">
                      <div className="min-w-0">
                        <p
                          className={`font-display text-lg mb-1.5 ${
                            p.accent ? "text-beige-100" : "text-ink"
                          }`}
                          style={{ fontWeight: 700 }}
                        >
                          {p.name}
                        ...
                        <p
                          className={`text-sm leading-relaxed ${
                            p.accent ? "text-beige-100/60" : "text-ink-mute"
                          }`}
                        >
                          {p.desc}
                        </p>
                      ...
                      <p
                        className={`tabular-nums shrink-0 font-display ${
                          p.accent
                            ? "text-beige-100 text-2xl"
                            : "text-ink-mute text-base line-through"
                        }`}
                        style={{ fontWeight: 700 }}
                      >
                        {p.price}
              ...
              <p className="text-center text-sm text-ink-mute italic">
                전통 자서전 대필 대비{" "}
                <strong className="text-ink not-italic">200분의 1</strong> 가격
              </p>
            </Reveal>
          </div>
        </section>
